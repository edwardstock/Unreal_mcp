...
public:
	...
	TSharedPtr<FJsonObject> BuildInputSchema() const override
	{
		return FMcpSchemaBuilder()
			...
			.String(TEXT("propertyPath"), TEXT("Dot-separated path into a struct or nested property (e.g., 'Transform.Location.X'). Use when 'propertyName' alone cannot address the target."))
			.FreeformObject(TEXT("value"), TEXT("New value for set_property / set_component_property. Type matches the target property: number, string, boolean, array, or object."))
			.String(TEXT("actorName"), TEXT("Name of the actor."))
			.String(TEXT("name"), TEXT("Name identifier."))
			.String(TEXT("componentName"), TEXT("Name of the component."))
			.String(TEXT("className"), TEXT("UClass name (without /Script/ prefix) for inspect_class / inspect_cdo / find_by_class (e.g., 'StaticMeshActor')."))
			.String(TEXT("classPath"), TEXT("Asset path (e.g., /Game/Path/Asset)."))
			.String(TEXT("tag"), TEXT("Name of the tag."))
			.String(TEXT("filter"), TEXT("Substring or wildcard filter applied to object names for list_objects / find_by_class."))
			.String(TEXT("snapshotName"), TEXT("Identifier for create_snapshot / restore_snapshot. Snapshots capture an object's serialized state."))
			.String(TEXT("destinationPath"), TEXT("Destination path for move/copy."))
			.String(TEXT("outputPath"), TEXT("Output file or directory path."))
			.String(TEXT("format"), TEXT("Output format for export action: 'json' (default), 'text', or 'csv' depending on target."))
			.String(TEXT("blueprintPath"), TEXT("Blueprint asset path."))
			.Bool(TEXT("detailed"), TEXT("When true, *_details actions return extended property readback (component sub-properties, transient fields, etc.). Defaults to false."))
			.Array(TEXT("propertyNames"), TEXT("Whitelist of property names to read in batch via get_property. Empty means all readable properties."))
