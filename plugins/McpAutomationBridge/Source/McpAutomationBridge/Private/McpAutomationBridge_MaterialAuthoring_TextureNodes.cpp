    ...
    TSharedPtr<FMcpBridgeWebSocket> Socket)
{
  ...
  if (SubAction == TEXT("add_texture_object"))
  {
    ...
    if (!Payload->TryGetStringField(TEXT("texturePath"), TexturePath) || TexturePath.IsEmpty()) {
      SendAutomationError(Socket, RequestId, TEXT("Missing 'texturePath'."), TEXT("missing-field")); return true;
    }
    ...
    UTexture* Texture = LoadObject<UTexture>(nullptr, *TexturePath);
    if (!Texture) { SendAutomationError(Socket, RequestId, TEXT("Could not load texture asset."), TEXT("invalid-asset")); return true; }
    ...
    UMaterialExpressionTextureObject* TexObj = NewObject<UMaterialExpressionTextureObject>(
        ...
        UMaterialExpressionTextureObject::StaticClass(), NAME_None, RF_Transactional);
    TexObj->Texture = Texture;
    float X = 0, Y = 0;
    Payload->TryGetNumberField(TEXT("x"), X); Payload->TryGetNumberField(TEXT("y"), Y);
    TexObj->MaterialExpressionEditorX = (int32)X; TexObj->MaterialExpressionEditorY = (int32)Y;
  ...
  if (SubAction == TEXT("add_texture_object_parameter"))
  {
    ...
    if (!Payload->TryGetStringField(TEXT("parameterName"), ParameterName) || ParameterName.IsEmpty()) {
      SendAutomationError(Socket, RequestId, TEXT("Missing 'parameterName'."), TEXT("missing-field")); return true;
    }
    if (!Payload->TryGetStringField(TEXT("texturePath"), TexturePath) || TexturePath.IsEmpty()) {
      SendAutomationError(Socket, RequestId, TEXT("Missing 'texturePath'."), TEXT("missing-field")); return true;
    }
    ...
    UMaterialExpressionTextureObjectParameter* TexParam = NewObject<UMaterialExpressionTextureObjectParameter>(
        GraphOwner.GraphSource ? GraphOwner.GraphSource : GraphOwner.Asset,
        UMaterialExpressionTextureObjectParameter::StaticClass(), NAME_None, RF_Transactional);
    TexParam->ParameterName = FName(*ParameterName);
    TexParam->Texture = Texture;
    FString Group; Payload->TryGetStringField(TEXT("group"), Group);
    if (!Group.IsEmpty()) TexParam->Group = FName(*Group);
    double SortPriority = 0; Payload->TryGetNumberField(TEXT("sortPriority"), SortPriority);
    TexParam->SortPriority = (int32)SortPriority;
    float X = 0, Y = 0;
    Payload->TryGetNumberField(TEXT("x"), X); Payload->TryGetNumberField(TEXT("y"), Y);
    TexParam->MaterialExpressionEditorX = (int32)X; TexParam->MaterialExpressionEditorY = (int32)Y;
  ...
  if (SubAction == TEXT("add_texture_sample_parameter"))
  {
    ...
    float X = 0, Y = 0;
    Payload->TryGetNumberField(TEXT("x"), X); Payload->TryGetNumberField(TEXT("y"), Y);
    TexSampleParam->MaterialExpressionEditorX = (int32)X; TexSampleParam->MaterialExpressionEditorY = (int32)Y;
    ...
    ParamInfo->SetNumberField(TEXT("sortPriority"), (double)SortPriority);
    Result->SetObjectField(TEXT("parameterInfo"), ParamInfo);
    Result->SetStringField(TEXT("samplerType"), TexSampleParam->SamplerType == SAMPLERTYPE_Color ? TEXT("Color") :
        TexSampleParam->SamplerType == SAMPLERTYPE_Normal ? TEXT("Normal") :
        TexSampleParam->SamplerType == SAMPLERTYPE_Masks ? TEXT("Masks") :
        TexSampleParam->SamplerType == SAMPLERTYPE_Grayscale ? TEXT("Grayscale") : TEXT("Default"));
    Result->SetBoolField(TEXT("saved"), bSaved);
    Result->SetBoolField(TEXT("dirty"), GraphOwner.Asset->GetOutermost()->IsDirty());
